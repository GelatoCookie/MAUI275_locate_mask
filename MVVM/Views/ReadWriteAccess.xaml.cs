...
namespace MauiRfidSample.MVVM.Views
{
    ...
    public partial class ReadWriteAccess : ContentPage
    {
        ...
        private void WriteOperation(object sender, EventArgs e)
        {
            ...
            // Validate that data.Text contains only hexadecimal characters
            if (!System.Text.RegularExpressions.Regex.IsMatch(data.Text, hexPattern))
            {
                DisplayAlert("Invalid Parameter", "Data must be a valid hexadecimal string", "Cancel");
                return;
            }
