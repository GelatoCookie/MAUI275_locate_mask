...
namespace MauiRfidSample.MVVM.ViewModels
{
    public class ReadWriteOperationsModel : BaseViewModel
    {
        ...
        public void AccessOperationsReadClicked()
        {
            ...
            if (ValidateFields())
            {
                ...
                ThreadPool.QueueUserWorkItem(o =>
                {
                    try
                    {
                        if (rfid.rfidReader == null)
                            return;
                        if(TagId.Length >= 24)
                            prefilter = true;
                        TagData tagData = rfid.rfidReader.Actions.TagAccess.ReadWait(TagId, readAccessParams, null,  prefilter);
        ...
        public async void AccessOperationsWriteClicked()
        {
            string TagId = TagPattern;
            UpdateBank();
            if (ValidateFields())
            {
                ...
                try
                {
                    await Task.Run(() =>
                    {
                        rfid.rfidReader.Actions.TagAccess.WriteWait(TagId, writeAccessParams, null, tagData, prefilter, false);
                    });

                  
                    // Check the operation status after WriteWait completes
        ...
        public void AccessOperationsLockClicked()
        {
            ...
            if (ValidateFields())
            {
                ...
                ThreadPool.QueueUserWorkItem(o =>
                {
                    try
                    {
                        rfid.rfidReader.Actions.TagAccess.LockWait(TagId, lockAccessParams, null, prefilter);
