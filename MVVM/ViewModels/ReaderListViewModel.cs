    ...
    public class ReaderListViewModel : BaseViewModel
    {
        ...
        private void UpdateReaderList()
        {
            ...
            for (int i = 0; i < readerlist.Count; i++)
            {
                ...
                {
                    readerDevice = readerlist[i];
                    model = readerDevice.GetDeviceCapability(readerDevice.Name);    
        ...
        public override void ReaderConnectionEvent(bool connection)
        {
            ...
            if (connection)
            {
                ...
                Device.BeginInvokeOnMainThread(() =>
                {
                    ...
                    if (rfidModel?.rfidReader?.HostName != null)
                    {
                ...
                if (readerlist == null)
                {
                    Console.WriteLine("Readerlist is null");
                    return;

                }
                if (readerlist != null)
                {
                    Console.WriteLine("Readerlist not null " + readerlist.Count);
                }
