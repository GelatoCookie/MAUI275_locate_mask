...
namespace MauiRfidSample.MVVM.ViewModels
{
    class ReaderWiFiViewModel : BaseViewModel
    {
        ...
        private string _connectedProfile, _addSSID, _passwordEntry,  _caCertSelected,
            _identityEntry, _clientCertSelected, _privateKeySelected, _anIdentityEntry;
        private string _eAPSelected ;
        ...
        public ReaderWiFiViewModel()
        {
            ...
            RFIDResults result = rfidModel.ScanWiFi();
            if (result ==  RFIDResults.AdminConnectError)
            {
                System.Diagnostics.Debug.WriteLine("AdminConnectError");
        ...
        internal void SelectedAvilableProfile(ItemTappedEventArgs e)
        {
            ...
            foreach(WifiScanData data in wifiScanDataList)
            {
                ...
                {
                    System.Diagnostics.Debug.WriteLine("Wifi Feature not supported");

                    Device.BeginInvokeOnMainThread(() =>
                    {
                        Toast.MakeText(Android.App.Application.Context, "Add Wifi profile feature is not supported", ToastLength.Short).Show();
                    });
                    return;
        ...
        {
            wifiScanDataList.Clear();
            RFIDResults rFIDResults = rfidModel.ScanWiFi();
            if(rFIDResults == RFIDResults.AdminConnectError)
            {
                System.Diagnostics.Debug.WriteLine("AdminConnectError");
                Shell.Current.GoToAsync("..");
                Application.Current.MainPage.Navigation.PopAsync();
        ...
        internal void AddProflie()
        {
            ...
            if (ProtocolSelected.ToString().Contains("Enterprise") && string.IsNullOrEmpty(EAPSelected))
            {
                Device.BeginInvokeOnMainThread(() =>
                {
                    Toast.MakeText(Android.App.Application.Context, "Select EAP", ToastLength.Short).Show();
                });
                return;
            }
