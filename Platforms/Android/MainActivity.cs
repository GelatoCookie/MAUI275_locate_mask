...
namespace MauiRfidSample
{
    ...
    public class MainActivity : MauiAppCompatActivity
    {
        ...
        internal void RequestUSBPermission()
        {
            ...
            try
            {
                var usbManager = (UsbManager)Android.App.Application.Context.GetSystemService(UsbService);
                if (usbManager == null || usbManager.DeviceList == null || usbManager.DeviceList.Count == 0)
                {
                    // No USB manager or devices; proceed with BT permission flow
                    CheckBTPermission();
                    return;
                }
                ...
                foreach (var dev in usbManager.DeviceList.Values)
                {
                ...
                {
                    // No matching Zebra device found; proceed with BT permission flow
                    CheckBTPermission();
                    return;
                }
                ...
                var permissionIntent = PendingIntent.GetBroadcast(this, 0, new Intent(ACTION_USB_PERMISSION), flags);

                // Register receiver once for this permission action
                var filter = new IntentFilter(ACTION_USB_PERMISSION);
                try
                {
                    Android.App.Application.Context.RegisterReceiver(new UsbReceiver(), filter);
                }
                catch (Exception)
                {
                    // If already registered or any issue, continue to request permission
                }
            ...
            {
                System.Diagnostics.Debug.WriteLine($"RequestUSBPermission error: {ex}");
                // Fallback to BT permission flow in case of any error
                CheckBTPermission();
            }
        }
